...
export default async function TerritoriesPage() {
  ...
  // Canvassers work from their assigned map, not the organization-wide territory manager.
  ...
  if (!workspace.campaignId) {
    return (
      <AppShell workspace={workspace} title="Territories">
        <div className="alert error">Create a campaign first.</div>
      </AppShell>
    );
  }
  ...
  const canManage = ["owner", "admin", "coordinator", "team_lead"].includes(
    workspace.role
  );
  ...
  return (
    <AppShell workspace={workspace} title="Territories">
      <div className="page-head">
        <div>
          <h1>Territories & assignments</h1>
          <p>
            Draw an area, attach every address inside it, and optionally assign the
            territory to a team member.
          </p>
        </div>
      </div>
      ...
      {canManage ? (
        ...
        <div className="alert">
          Your role can view territories but cannot create assignments.
        </div>
      )}
              ...
              {territories.length === 0 ? (
                <tr>
                  <td colSpan={3} className="muted">
                    No territories yet. Draw the first one above.
                  </td>
                </tr>
              ) : null}
            </tbody>
