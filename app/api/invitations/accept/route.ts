...
export async function POST() {
  try {
    ...
    if (error || !data.user) {
      return NextResponse.json({ error: "Your invitation session is not available." }, { status: 401 });
    }

    const user = data.user;
    const invitationId = user.user_metadata?.organization_invitation_id as string | undefined;
    if (!invitationId) {
      return NextResponse.json({ error: "This account does not contain an organization invitation." }, { status: 400 });
    }
