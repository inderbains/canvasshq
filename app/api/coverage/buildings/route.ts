...
async function fetchBuildings(
  ...
  for (let offset = 0; offset < 2000; offset += pageSize) {
    ...
    const response = await fetch(`${BUILDING_URL}?${params}`, {
      cache: "no-store",
    });

    if (!response.ok) {
      throw new Error("Could not load Surrey building footprints.");
    }
...
export async function GET(request: Request) {
  try {
    ...
    // Keep this endpoint street-level. A wide query can exceed the GIS service limit.
    if (bbox.xmax - bbox.xmin > 0.035 || bbox.ymax - bbox.ymin > 0.035) {
      return NextResponse.json(
        {
          error: "Zoom in closer to review missing buildings.",
          code: "ZOOM_IN",
        },
        { status: 400 },
      );
    ...
    if (!membership || !managerRoles.has(membership.role)) {
      return NextResponse.json(
        { error: "Manager access is required for coverage review." },
        { status: 403 },
      );
    }
    ...
    for (const building of buildings) {
      ...
      // Only inspect nearby points before doing polygon checks.
      for (const address of addresses) {
        if (
          Math.abs(address.latitude - centroid.latitude) > 0.00045 ||
          Math.abs(address.longitude - centroid.longitude) > 0.00065
        ) {
          continue;
        }
      ...
      // Address points are sometimes slightly outside the building footprint.
      const hasNearbyAddress = containsAddress || nearestDistance <= 18;
      if (hasNearbyAddress) {
        matched += 1;
        continue;
      }
    ...
    return NextResponse.json({
      ...
      note:
        "Orange candidates are building footprints without a nearby campaign address point. Verify before adding a door.",
    });
  } catch (error) {
    return NextResponse.json(
      {
        error:
          error instanceof Error
            ? error.message
            : "Could not review building coverage.",
      },
      { status: 500 },
    );
  }
