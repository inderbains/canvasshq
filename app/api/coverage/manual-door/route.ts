...
function parseAddress(fullAddress: string) {
  ...
  if (!match) {
    return { houseNumber: null, roadName: null };
  }

  return {
    houseNumber: Number(match[1]),
    roadName: match[2].trim(),
  };
}
...
export async function POST(request: Request) {
  try {
    ...
    if (!membership || !managerRoles.has(membership.role)) {
      return NextResponse.json(
        { error: "Manager access is required to add a door." },
        { status: 403 },
      );
    }
    ...
    // Stop obvious accidental duplicates before creating a manual door.
    ...
    if ((nearby ?? []).some((address) => address.full_address.toLowerCase() === fullAddress.toLowerCase())) {
      return NextResponse.json(
    ...
    return NextResponse.json(
      {
        error:
          error instanceof Error ? error.message : "Could not add the manual door.",
      },
      { status: 500 },
    );
  }
